...
import "../styles.scss";
import { donateProjectService } from "../../../services/transaction.service";
...
const DonateProject = () => {
  ...
  const onFinish = async (values) => {
    ...
    await donateProjectService(data)
      .then((res) => {
        if (res.status === 204) {
          message.error("Donate failed! Something is wrong");
        }
        if (res.status === 201) {
          message.success(`Your are donated ${data.amount} successfull!`);
